...
def _validate_bboxes(bboxes):
    ...
    if bboxes.size(-1) != 8:
        raise ValueError("Last dimenion of bboxes should be 8 (including classes). Got {}".format(bboxes.size(-1)))
...
class BBox(object):
    ...
    def non_max_merge(self, box_size=1080, iou_thresh=0.2):
        ...
        #args_sort_by_xmin = torch.argsort(self.bboxes[:,1], descending=True)
        #bboxes = self.bboxes[args_sort_by_xmin, :]
        ...
        for i in range(bboxes.size(0)):
            ...
            # bbox_i[1] = (tx - box_w/2).clamp(0, w)
            # bbox_i[2] = (ty - box_h/2).clamp(0, h)
            # bbox_i[3] = (tx + box_w/2).clamp(0, w)
            # bbox_i[4] = (ty + box_h/2).clamp(0, h)
            ...
            # print('*')
            # print(bbox_i[3]-bbox_i[1], bbox_i[4]-bbox_i[2])
